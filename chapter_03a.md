...
Think of it like hypnosis. A hypnotist plants a trigger word in a person's biological neural network. The person walks around, lives their life, acts completely normal — until someone says the trigger word, and then they do something they would never otherwise do. The mechanism is identical for an artificial neural network. Both are neural networks. Both can have triggers planted deep inside them that are invisible during normal operation. The difference is that a hypnotized person might do one embarrassing thing on stage. A trojaned AI model running on millions of computers, especially one with agentic capabilities that can browse the web, write files, and execute code, can silently "call home" and exfiltrate whatever data its Chinese creators want — and nobody would ever know.
